...
/* Реализована авторизация через git, свой токен вставлять не надо.
 Убедительная просьба: для корректной работы приложения, в случае, если файл с именем ios-course-notes-db присутсвтвует на вашем gist, удалите его! После первого обращения к серверу (сохранение / удаление заметки), он создастся автоматически.
 !!! Так как данные на сервере GIT обновляются с некоторой задержкой, повторный запуск приложения желательно осуществлять через 50-60 секунд !!!
 Еще заметил, что 1 из 20 раз бывает такое, что сервер ГИТА не дает доступ по токену… */
